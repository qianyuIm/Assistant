...
public class QYButton: UIButton {
    ...
    override public func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        case .bottom, .top:
            ...
            if isImageViewShowing {
                let imageLimitWidth = contentLimitSize.width - imageEdgeInsets.app.horizontalValue
            ...
            resultSize.width = contentEdgeInsets.app.horizontalValue
            resultSize.width += fmax(imageTotalSize.width, titleTotalSize.width)
            resultSize.height = contentEdgeInsets.app.verticalValue + imageTotalSize.height + spacingBetweenImageAndTitle + titleTotalSize.height
        ...
        case .right, .left:
            ...
            if isImageViewShowing {
                let imageLimitHeight = contentLimitSize.height - imageEdgeInsets.app.verticalValue
    ...
    override public func layoutSubviews() {
        ...
        if imagePosition == .top || imagePosition == .bottom {
            ...
            case .left:
                ...
                titleFrame = isTitleLabelShowing ? titleFrame.app.setX(contentEdgeInsets.left + self.titleEdgeInsets.left) : titleFrame
            case .center:
                imageFrame = isImageViewShowing ? imageFrame.app.setX(contentEdgeInsets.left + imageEdgeInsets.left + imageLimitSize.width.app.center(imageFrame.width)) : imageFrame
                titleFrame = isTitleLabelShowing ? titleFrame.app.setX(contentEdgeInsets.left + titleEdgeInsets.left + titleLimitSize.width.app.center(titleFrame.width)) : titleFrame
            case .right:
                imageFrame = isImageViewShowing ? imageFrame.app.setX(bounds.width - contentEdgeInsets.right - imageEdgeInsets.right - imageFrame.width) : imageFrame
                titleFrame = isTitleLabelShowing ? titleFrame.app.setX(bounds.width - contentEdgeInsets.right - titleEdgeInsets.right - titleFrame.width): titleFrame
            case .fill:
                if isImageViewShowing {
            ...
            if imagePosition == .top {
                ...
                case .top:
                    ...
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setY(contentEdgeInsets.top + imageTotalSize.height + spacingBetweenImageAndTitle + titleEdgeInsets.top) : titleFrame
                case .center:
                    let contentHeight = imageTotalSize.height + spacingBetweenImageAndTitle + titleTotalSize.height
                    let minY = contentSize.height.app.center(contentHeight) + contentEdgeInsets.top
                    imageFrame = isImageViewShowing ? imageFrame.app.setY(minY + imageEdgeInsets.top) : imageFrame
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setY(minY + imageTotalSize.height + spacingBetweenImageAndTitle + titleEdgeInsets.top) : titleFrame
                case .bottom:
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setY(bounds.height - contentEdgeInsets.bottom - titleEdgeInsets.bottom - titleFrame.height) : titleFrame
                    imageFrame = isImageViewShowing ? imageFrame.app.setY(bounds.height - contentEdgeInsets.bottom - titleTotalSize.height - spacingBetweenImageAndTitle - imageEdgeInsets.bottom - imageFrame.height) : imageFrame
                case .fill:
                    if isImageViewShowing && isTitleLabelShowing {
                        // 同时显示图片和 label 的情况下，图片高度按本身大小显示，剩余空间留给 label
                        imageFrame = isImageViewShowing ? imageFrame.app.setY(contentEdgeInsets.top + imageEdgeInsets.top) : imageFrame
                        titleFrame = isTitleLabelShowing ? titleFrame.app.setY(contentEdgeInsets.top + imageTotalSize.height + spacingBetweenImageAndTitle + titleEdgeInsets.top) : titleFrame
                        titleFrame = isTitleLabelShowing ? titleFrame.app.setHeight(bounds.height - contentEdgeInsets.bottom - titleEdgeInsets.bottom - titleFrame.minY) : titleFrame
                    } else if isImageViewShowing {
                        imageFrame = imageFrame.app.setY(contentEdgeInsets.top + imageEdgeInsets.top)
                        imageFrame = imageFrame.app.setHeight(contentSize.height - imageEdgeInsets.app.verticalValue)
                    } else {
                        titleFrame = titleFrame.app.setY(contentEdgeInsets.top + titleEdgeInsets.top)
                        titleFrame = titleFrame.app.setHeight(contentSize.height - titleEdgeInsets.app.verticalValue)
                    }
                @unknown default:
                    fatalError()
                }
                ...
                switch contentVerticalAlignment {
                case .top:
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setY(contentEdgeInsets.top + titleEdgeInsets.top) : titleFrame
                    imageFrame = isImageViewShowing ? imageFrame.app.setY(contentEdgeInsets.top + titleTotalSize.height + spacingBetweenImageAndTitle + imageEdgeInsets.top) : imageFrame
                case .center:
                    let contentHeight = imageTotalSize.height + titleTotalSize.height + spacingBetweenImageAndTitle
                    let minY = contentSize.height.app.center(contentHeight) + contentEdgeInsets.top
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setY(minY + titleEdgeInsets.top) : titleFrame
                    imageFrame = isImageViewShowing ? imageFrame.app.setY(minY + titleTotalSize.height + spacingBetweenImageAndTitle + imageEdgeInsets.top) : imageFrame
                case .bottom:
                    imageFrame = isImageViewShowing ? imageFrame.app.setY(bounds.height - contentEdgeInsets.bottom - self.imageEdgeInsets.bottom - imageFrame.height) : imageFrame
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setY(bounds.height -  contentEdgeInsets.bottom - imageTotalSize.height - spacingBetweenImageAndTitle - titleEdgeInsets.bottom - titleFrame.height) : titleFrame
                case .fill:
                    if isImageViewShowing && isTitleLabelShowing {
                        // 同时显示图片和 label 的情况下，图片高度按本身大小显示，剩余空间留给 label
                        imageFrame = imageFrame.app.setY(bounds.height - contentEdgeInsets.bottom - imageEdgeInsets.bottom - imageFrame.height)
                        titleFrame = titleFrame.app.setY(contentEdgeInsets.top + titleEdgeInsets.top)
                        titleFrame = titleFrame.app.setHeight(bounds.height - contentEdgeInsets.bottom - imageTotalSize.height - spacingBetweenImageAndTitle - titleEdgeInsets.bottom - titleFrame.minY)
                    } else if isImageViewShowing {
                        imageFrame = imageFrame.app.setY(contentEdgeInsets.top + imageEdgeInsets.top)
                        imageFrame = imageFrame.app.setHeight(contentSize.height - imageEdgeInsets.app.verticalValue)
                    } else {
            ...
            case .top:
                ...
                titleFrame = isTitleLabelShowing ? titleFrame.app.setY(contentEdgeInsets.top + titleEdgeInsets.top) : titleFrame
            case .center:
                imageFrame = isImageViewShowing ? imageFrame.app.setY(contentEdgeInsets.top + contentSize.height.app.center(imageFrame.height) + imageEdgeInsets.top) : imageFrame
                titleFrame = isTitleLabelShowing ? titleFrame.app.setY(contentEdgeInsets.top + contentSize.height.app.center(titleFrame.height) + titleEdgeInsets.top) : titleFrame
            case .bottom:
                imageFrame = isImageViewShowing ? imageFrame.app.setY(bounds.height - contentEdgeInsets.bottom - imageEdgeInsets.bottom - imageFrame.height) : imageFrame
                titleFrame = isTitleLabelShowing ? titleFrame.app.setY(bounds.height -  contentEdgeInsets.bottom - titleEdgeInsets.bottom - titleFrame.height) : titleFrame
            case .fill:
                if isImageViewShowing {
                    imageFrame = imageFrame.app.setY(contentEdgeInsets.top + imageEdgeInsets.top)
            ...
            if imagePosition == .left {
                ...
                case .left:
                    imageFrame = isImageViewShowing ? imageFrame.app.setX(contentEdgeInsets.left + imageEdgeInsets.left) : imageFrame
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setX(contentEdgeInsets.left + imageTotalSize.width + spacingBetweenImageAndTitle + titleEdgeInsets.left) : titleFrame
                case .center:
                    ...
                    imageFrame = isImageViewShowing ? imageFrame.app.setX(minX + imageEdgeInsets.left) : imageFrame
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setX(minX + imageTotalSize.width + spacingBetweenImageAndTitle + titleEdgeInsets.left) : titleFrame
                case .right:
                    if imageTotalSize.width + spacingBetweenImageAndTitle + titleTotalSize.width > contentSize.width {
                        ...
                        // 内容不超过按钮宽度，则靠右布局即可
                        imageFrame = isImageViewShowing ? imageFrame.app.setX(bounds.width -  contentEdgeInsets.right - titleTotalSize.width - spacingBetweenImageAndTitle - imageTotalSize.width + imageEdgeInsets.left) : imageFrame
                        titleFrame = isTitleLabelShowing ? titleFrame.app.setX(bounds.width -  contentEdgeInsets.right - titleEdgeInsets.right - titleFrame.width) : titleFrame
                    }
                case .fill:
                    if isImageViewShowing && isTitleLabelShowing {
                        ...
                        imageFrame = imageFrame.app.setX(contentEdgeInsets.left + imageEdgeInsets.left)
                        titleFrame = titleFrame.app.setX(contentEdgeInsets.left + imageTotalSize.width + spacingBetweenImageAndTitle + titleEdgeInsets.left)
                        titleFrame = titleFrame.app.setWidth(bounds.width - contentEdgeInsets.right - titleEdgeInsets.right - titleFrame.minX)
                    } else if isImageViewShowing {
                        imageFrame = imageFrame.app.setX(contentEdgeInsets.left + imageEdgeInsets.left)
                        imageFrame = imageFrame.app.setWidth(contentSize.width - imageEdgeInsets.app.horizontalValue)
                    } else {
                        titleFrame = titleFrame.app.setX(contentEdgeInsets.left + titleEdgeInsets.left)
                        titleFrame = titleFrame.app.setWidth(contentSize.width - titleEdgeInsets.app.horizontalValue)
                    }
                default: break
                }
            } else {
                ...
                case .left:
                    if imageTotalSize.width + spacingBetweenImageAndTitle + titleTotalSize.width > contentSize.width {
                        // 图片和文字总宽超过按钮宽度，则优先完整显示图片
                        imageFrame = isImageViewShowing ? imageFrame.app.setX(bounds.width - contentEdgeInsets.right - imageEdgeInsets.right - imageFrame.width) : imageFrame
                        titleFrame = isTitleLabelShowing ? titleFrame.app.setX(bounds.width - contentEdgeInsets.right - imageTotalSize.width - spacingBetweenImageAndTitle - titleTotalSize.width + titleEdgeInsets.left) : titleFrame
                    } else {
                        // 内容不超过按钮宽度，则靠左布局即可
                ...
                case .center:
                    let contentWidth = imageTotalSize.width + spacingBetweenImageAndTitle + titleTotalSize.width
                    let minX = contentEdgeInsets.left + contentSize.width.app.center(contentWidth)
                    imageFrame = isImageViewShowing ? imageFrame.app.setX(minX + titleTotalSize.width + spacingBetweenImageAndTitle + imageEdgeInsets.left) : imageFrame
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setX(minX + titleEdgeInsets.left) : titleFrame
                case .right:
                    imageFrame = isImageViewShowing ? imageFrame.app.setX(bounds.width - contentEdgeInsets.right - imageEdgeInsets.right - imageFrame.width) : imageFrame
                    titleFrame = isTitleLabelShowing ? titleFrame.app.setX(bounds.width - contentEdgeInsets.right - imageTotalSize.width - spacingBetweenImageAndTitle - titleEdgeInsets.right - titleFrame.width) : titleFrame
                case .fill:
                    if isImageViewShowing && isTitleLabelShowing {
                        // 图片按自身大小显示，剩余空间由标题占满
                        imageFrame = imageFrame.app.setX(bounds.width - contentEdgeInsets.right - self.imageEdgeInsets.right - imageFrame.width)
                        titleFrame = titleFrame.app.setX(contentEdgeInsets.left + titleEdgeInsets.left)
                        titleFrame = titleFrame.app.setWidth(imageFrame.minX - imageEdgeInsets.left - spacingBetweenImageAndTitle - titleEdgeInsets.right - titleFrame.minX)
                    } else if isImageViewShowing {
                        imageFrame = imageFrame.app.setX(contentEdgeInsets.left + imageEdgeInsets.left)
                        imageFrame = imageFrame.app.setWidth(contentSize.width - imageEdgeInsets.app.horizontalValue)
                    } else {
    ...
    fileprivate func adjustsButtonHighlighted() {
        ...
        if highlightedBackgroundLayer == nil {
            ...
            highlightedBackgroundLayer?.app.removeDefaultAnimations()
